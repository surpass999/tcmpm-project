...
export interface IDialogOptions {
  onClose?: () => void
  onCancel?: () => void
  onConfirm?: (payload: IDialogConfirm[], action?: string) => void
  title: string
  data: IDialogData[]
...
export class Dialog {
  ...
  private _render() {
    const { title, data, onClose, onCancel, onConfirm, buttons } = this.options
    // 渲染遮罩层
    ...
    // 如果没有自定义按钮，使用默认按钮
    const defaultButtons: IDialogButton[] = [
      { text: '取消', type: 'cancel', action: 'cancel' },
    ...
    buttonList.forEach(button => {
      ...
      btn.classList.add('dialog-menu__btn', `dialog-menu__${button.type || 'default'}`)
      ...
      btn.onclick = () => {
        if (button.action === 'cancel') {
          ...
          if (onConfirm) {
            ...
            onConfirm(payload, button.action)
          }
        }
        this._dispose()
      }
    ...
    // 设置定位
    if (this.options.left !== undefined || this.options.top !== undefined) {
