...
export class Signature {
  ...
  private _bindEvent() {
    if (this.trashContainer) this.trashContainer.onclick = this._clearCanvas.bind(this)
    if (this.undoContainer) this.undoContainer.onclick = this._undo.bind(this)
    if (this.canvas) {
  ...
  private _undo() {
    if (this.undoStack.length > 1) {
      this.undoStack.pop()
      const fn = this.undoStack[this.undoStack.length - 1]!
      if (fn) {
        try {
          fn()
        } catch (e) {
