...
import type { ILocationPosition, IPositionContext } from '../../interface/Position'
...
export class CommandAdapt {
  ...
  public setPositionContext(range: IRange) {
    ...
    if (tableId) {
      ...
      if (!tableElement || !tableElement.trList || startTrIndex == null || startTdIndex == null) {
        return
      }
      const tr = tableElement.trList[startTrIndex]!
      if (!tr || !tr.tdList) return
  ...
  public cancelHyperlink() {
    ...
    for (let i = leftIndex; i <= rightIndex; i++) {
      ...
      delete (element as any).type
      delete (element as any).url
      delete (element as any).hyperlinkId
      delete (element as any).underline
    }
    this.draw!.getHyperlinkParticle().clearHyperlinkPopup()
    // 重置画布
    const { endIndex } = this.range!.getRange()
  ...
  public addWatermark(payload: IWatermark) {
    ...
    try {
      console.log('[CommandAdapt] addWatermark options.watermark =', options.watermark)
    } catch {}
    this.draw!.render({
      isSetCursor: false,
      isSubmitHistory: false,
  ...
  public getRangeContext(): RangeContext | null {
    ...
    const startElement = pickElementAttr(
      elementList[isCollapsed ? startIndex : startIndex + 1]! || {} as IElement,
      {
        extraPickAttrs: ['id', 'controlComponent']
      }
    )
    const endElement = pickElementAttr(elementList[endIndex]! || {} as IElement, {
      extraPickAttrs: ['id', 'controlComponent']
    })
    // 页码信息、行信息
    ...
    if (selectionPositionList) {
      ...
      rangeRects.push({
        x: rightTop?.[0] ?? 0,
        y: (rightTop?.[1] ?? 0) + pageNo * (height + pageGap),
        width: 0,
        height: lineHeight
      })
    }
    ...
    if (isTable) {
      ...
      if (originTableElement) {
        tableElement = zipElementList([originTableElement])[0] || null
      }
    }
    // 标题信息
