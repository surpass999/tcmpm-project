...
export class Draw {
  ...
  public getContextInnerWidth(): number {
    ...
    if (positionContext.isTable) {
      ...
      if (!td?.width) return 0
      const tdPadding = this.getTdPadding()
      return td.width - (tdPadding[1] ?? 0) - (tdPadding[3] ?? 0)
    }
  ...
  public getTableRowList(sourceElementList: IElement[]): IRow[] {
    ...
    const { index, trIndex, tdIndex } = positionContext
    const tableElement = sourceElementList[index!]
    const tr = tableElement?.trList?.[trIndex!]
    const td = tr?.tdList?.[tdIndex!]
    return td?.rowList || []
  }
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (
        ...
        if (isPagingMode) {
          ...
          const firstTrHeight = (element.trList?.[0]?.height ?? 0) * scale
          if (
    ...
    for (let i = 0; i < rowList.length; i++) {
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        if (highlight) {
          ...
          if (!position?.coordinate?.leftTop) continue
          const [x, y] = position.coordinate.leftTop
          // 元素向左偏移量
    ...
    for (let i = 0; i < rowList.length; i++) {
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        if (
          ...
          if (isCrossRowCol && x != null && y != null) {
            rangeRecord.x = x
            rangeRecord.y = y
        ...
        if (
          ...
          if (
            ...
            if (startIndex === index) {
              const nextElement = elementList[startIndex + 1]!
              if (nextElement && nextElement.value! === ZERO && x != null && y != null) {
              ...
              // 记录第一次位置、行高
              if (!rangeRecord.width && x != null && y != null) {
                rangeRecord.x = x
                rangeRecord.y = y
                rangeRecord.height = curRow.height
              }
      ...
      if (!isPrintMode && !isGraffitiMode) {
        if (rangeRecord.width && rangeRecord.height) {
          ...
          this.range.render(ctx, x ?? 0, y ?? 0, width ?? 0, height ?? 0)
        }
        if (
          isCrossRowCol &&
          tableRangeElement &&
          ...
          const position = positionList[curRow.startIndex!]
          if (position?.coordinate?.leftTop) {
            const [x, y] = position.coordinate.leftTop
            this.tableParticle.drawRange(ctx, tableRangeElement, x ?? 0, y ?? 0)
          }
        }
      }
    }
