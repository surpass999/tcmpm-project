  ...
  public replace(payload: string, option?: IReplaceOption) {
    ...
    for (let m = 0; m < matchList.length; m++) {
      ...
      if (match.type! === EditorContext.TABLE) {
        ...
        const tableElement = elementList[curTableIndex]
        const tr = tableElement?.trList?.[trIndex!]
        const td = tr?.tdList?.[tdIndex!]
        const tableElementList = td?.value || []
        // 表格内元素
        const curIndex = index + tableDiffCount
        const innerTableElement = tableElementList[curIndex]!
        // 非设计模式下设置元素不可删除 || 控件结构元素 => 禁止替换
        if (
          !isDesignMode &&
          (innerTableElement?.control?.deletable === false ||
            innerTableElement?.title?.deletable === false)
        ) {
          continue
        }
        if (payload === '') {
