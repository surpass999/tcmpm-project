...
function parse(tokens: string[]): Expr {
  ...
  for (i = 0; i < tokens.length; i++) {
    const s: Symb = SYMB[tokens[i]!] as Symb
    const e: Expr = {
      type: '',
      text: tokens[i]!,
      mode: 'math',
...
function align(exprs: Expr[], alignment = 'center'): void {
  for (let i = 0; i < exprs.length; i++) {
    if (exprs[i]!.text == '^' || exprs[i]!.text == '\'') {
      ...
      if (exprs[i]!.text == '\'') {
        ...
        const symbEntry = SYMB[exprs[j]!.text]
        if (symbEntry && symbEntry.flags?.big) {
  ...
  for (let i = 0; i < exprs.length; i++) {
    if (exprs[i]!.text == '\\left') {
      ...
      if (ymin != Infinity && ymax != -Infinity) {
        exprs[i]!.bbox.y = ymin ?? 0
        transform(exprs[i]!, 1, ((ymax ?? 0) - (ymin ?? 0)) / exprs[i]!.bbox.h, 0, 0)
      }
  ...
  for (let i = 0; i < erows.length; i++) {
    ...
    for (let j = 0; j < erows[i]!.length; j++) {
      ...
      const bbox = erows[i]![j].bbox
      ymin = Math.min(ymin, bbox?.y ?? 0)
      ymax = Math.max(ymax, (bbox?.y ?? 0) + (bbox?.h ?? 0))
    }
    ybds.push([ymin, ymax])
  }
  ...
  for (let i = 0; i < ybds.length; i++) {
    if (ybds[i]![0] == Infinity || ybds[i]![1] == Infinity) {
      ybds[i]![0] = i == 0 ? 0 : (ybds[i - 1]![1] ?? 0)
      ybds[i]![1] = (ybds[i]![0] ?? 0) + 2
    }
  }

  for (let i = 1; i < erows.length; i++) {
    const prevYMax = ybds[i - 1]?.[1] ?? 0
    const currYMin = ybds[i]?.[0] ?? 0
    const shft = prevYMax - currYMin + CONFIG.LINE_SPACING
    for (let j = 0; j < erows[i]!.length; j++) {
      if (erows[i]![j]) {
        erows[i]![j].bbox.y = (erows[i]![j].bbox.y ?? 0) + shft
      }
    }
    if (ybds[i]) {
      ybds[i][0] = (ybds[i][0] ?? 0) + shft
    }
    ybds[i]![1] += shft
  }
...
function plan(expr: Expr, mode = 'math'): void {
  ...
  if (expr.text == '\\frac') {
    ...
    const s: number = CONFIG.FRAC_SCALE ?? 1
    plan(a)
    plan(b)
    a.bbox.x = 0
    a.bbox.y = 0
    ...
    if (SYMB[e.text]! && SYMB[e.text]!.glyph) {
      ...
      for (let n = Number(isVerb); n < e.text.length; n++) {
        ...
        for (let j = 0; j < d.polylines.length; j++) {
          const l: number[][] = []
          for (let k = 0; k < d.polylines[j]!.length; k++) {
            let x = d.polylines[j]![k][0] ?? 0
            let y = d.polylines[j]![k][1] ?? 0
            x /= 16
            y /= 16
            x *= s
            y *= s
