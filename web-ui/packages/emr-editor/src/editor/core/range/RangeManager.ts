...
export class RangeManager {
    ...
    for (const searchMatch of searchMatchList) {
      ...
      if (searchRange) {
        ...
        if (type === EditorContext.TABLE) {
          ...
          range.startTdIndex! = tdIndex ?? 0
          range.endTdIndex = tdIndex ?? 0
          range.startTrIndex! = trIndex ?? 0
          range.endTrIndex = trIndex ?? 0
        }
  ...
  public setRange(
    ...
    if (isChange) {
      ...
      this.range.endIndex! = endIndex
      this.range.tableId! = tableId
      this.range.startTdIndex! = startTdIndex ?? 0
      this.range.endTdIndex = endTdIndex ?? 0
      this.range.startTrIndex! = startTrIndex ?? 0
      this.range.endTrIndex = endTrIndex ?? 0
