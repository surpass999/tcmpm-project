...
import { Button, Steps, Step, Spin } from 'ant-design-vue';
...
const returnModalRef = ref<{
  openModal: (data: { taskId: string; title?: string }) => void;
} | null>(null);
...
// 专家选择弹窗显示状态
const expertModalVisible = ref(false);
...
// 获取备案状态显示文本（基于 filingData.filingStatus，使用字典）
function getFilingStatusText(status?: string): string {
  if (!status) return '未知';
  // 优先使用字典获取标签
  const dict = getDictObj(DICT_TYPE.DECLARE_FILING_STATUS, status);
  if (dict?.label) return dict.label;
  // 兜底：根据状态值返回中文
  const statusMap: Record<string, string> = {
    'DRAFT': '草稿',
    'SUBMITTED': '已提交',
    'PROVINCE_APPROVED': '省级审核通过',
    'EXPERT_APPROVED': '专家论证通过',
    'NATION_APPROVED': '国家局审核通过',
    'ARCHIVED': '已归档',
    'RETURNED': '退回修改',
  };
...
// 获取备案状态样式类
function getFilingStatusClass(status?: string): string {
  if (!status) return 'status-tag-pending';
  switch (status) {
    case 'DRAFT':
      return 'status-tag-pending';    // 草稿 - 待处理样式
    case 'SUBMITTED':
      return 'status-tag-pending';    // 已提交 - 待处理样式
    case 'PROVINCE_APPROVED':
    case 'EXPERT_APPROVED':
    case 'NATION_APPROVED':
      return 'status-tag-success';   // 各种审核通过 - 成功样式
    case 'ARCHIVED':
      return 'status-tag-success';   // 已归档 - 成功样式
    case 'RETURNED':
      return 'status-tag-warning';  // 退回修改 - 警告样式
    default:
      return 'status-tag-pending';
  }
}

// 获取流程状态显示文本（基于 approvalDetail.status - 保留用于流程进度显示）
...
// 审批按钮类型：1=通过(primary)，其余=default（拒绝用 danger 属性）
function getActionButtonType(key: string | number): 'primary' | 'default' {
  const k = String(key);
  if (k === '1' || k.includes('pass') || k.includes('agree') || k.includes('approve')) {
    return 'primary';
  }
  return 'default';
}

// 是否为拒绝类操作（用于绑定 danger 属性使用系统 destructive 色）
function isRejectAction(key: string | number): boolean {
  const k = String(key);
  return k === '2' || k.includes('reject') || k.includes('refuse') || k.includes('deny');
}

// 获取按钮图标
function getActionIcon(key: string | number): string {
  const k = String(key);
...
function handleActionClick(action: BpmActionApi.BpmAction & { taskId?: string }) {
  ...
  if (isSimpleButton) {
    ...
    if (buttonType === OperationButtonType.SELECT_APPROVER) {
      currentAction.value = action;
      expertModalVisible.value = true;
      return;
    }

    // 发起整改操作 - 打开审批弹窗（填写整改意见）
    if (buttonType === OperationButtonType.RECTIFY) {
      currentAction.value = action;
      approvalModalRef.value?.open({
        action: 'approve',
        title: action.label || '发起整改',
        reason: vars.reason || '',
      });
      return;
    }
    ...
    if (vars.reason || vars.reasonRequired) {
      currentAction.value = action;
      // 根据是否有明确的拒绝含义判断 action 类型
      const isReject =
        action.label?.includes('拒绝') ||
        action.label?.includes('不通过') ||
        action.label?.includes('驳回');
      approvalModalRef.value?.open({
        action: isReject ? 'reject' : 'approve',
        title: action.label || '审批',
        reason: vars.reason || '',
      });
      return;
    }
...
// 专家选择确认 - 注意：函数名要与模板中的 @confirm 绑定一致
async function handleExpertConfirm(experts: any[]) {
  if (!currentAction.value) {
    message.error('操作信息丢失，请重试');
    return;
  }
...
// 用户选择确认（转办/委派/加签）
function handleUserSelectConfirm(_users: any[], reason: string) {
  if (!currentAction.value) return;
  executeAction(currentAction.value, reason);
}
  ...
  <div class="approval-detail-wrapper">
    ...
    <div v-else class="approval-detail-content">
      ...
      <div class="filing-status-section">
        <div class="filing-status-tag" :class="getFilingStatusClass(filingData?.filingStatus)">
          {{ getFilingStatusText(filingData?.filingStatus) }}
        </div>
    ...
    <ApprovalModal
      ...
      @confirm="(reason, action) => executeAction(currentAction!, reason)"
    />

    <!-- 退回弹窗 -->
    <ReturnModal
      ref="returnModalRef"
      @success="handleReturnConfirm"
    />
    ...
    <ExpertSelectModal
      ref="expertModalRef"
      v-model:open="expertModalVisible"
      :multiple="true"
      :expert-min="1"
      :expert-max="10"
      @confirm="(experts) => handleExpertConfirm(experts)"
    />
  </div>
  </template>
...
/* 全局覆盖 Ant Design Vue danger 按钮样式 */
