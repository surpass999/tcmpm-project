...
/**
 * 是否选中了某个输入型选项（考虑 formValues 和 lastPeriodValues）
 * 用于 checkbox 点击时判断是"取消"还是"添加"
 * @param indicatorCode - 指标编码
 * @param optionValue - 选项值
 * @param currentFormValues - 当前表单值（多选数组）
 * @param lastPeriodRaw - 上期原始值（字符串，如 "4,3"）
 */
export function isOptionSelectedWithLastPeriod(
  ...
  if (currentFormValues && Array.isArray(currentFormValues)) {
    return currentFormValues.some((v) => deserializeInputTypeValue(v).value === optionValue);
  }
