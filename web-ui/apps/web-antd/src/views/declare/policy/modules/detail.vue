...
const formData = ref<DeclarePolicyApi.Policy>();
...
const [Modal, modalApi] = useVbenModal({
  async onOpenChange(isOpen: boolean) {
    ...
    const data = modalApi.getData<DeclarePolicyApi.Policy>();
    if (!data || !data.id) {
      return;
    }
...
/** 获取政策类型标签 */
function getPolicyTypeLabel(type: number) {
  ...
  >
    ...
    <div class="mb-6 text-center border-b pb-4">
      ...
      <div class="flex flex-wrap justify-center gap-2 mb-3">
        <span
          v-if="formData?.policyType"
          class="px-3 py-1 text-white text-xs rounded-md"
          :class="formData.policyType === 1 ? 'bg-[#4A8F72]' : 'bg-[#A07852]'"
        >
          {{ getPolicyTypeLabel(formData.policyType) }}
        </span>
        <span
        ...
        <span
          ...
          class="px-3 py-1 text-white text-xs rounded-md"
          :class="getStatusInfo(formData.status).color === 'green' ? 'bg-green-600' : 'bg-red-500'"
        >
